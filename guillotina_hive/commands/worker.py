        ...
        if payload_config is False:
            ...
            if IDatabase.providedBy(db_obj):
                ...
                if IContainer.providedBy(container_obj):
                    guillotina_execution = await navigate_to(container_obj, '/'.join(elements[2:]))  # noqa
                    if IExecution.providedBy(guillotina_execution):
                        task_obj = TaskObject(data=guillotina_execution.get_task_payload())  # noqa
